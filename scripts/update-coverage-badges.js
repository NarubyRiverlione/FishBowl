...
const updateReadmeBadges = (coverageStats, dryRun = false) => {
  ...
  const badges = [
    {
      pattern: /!\[Coverage: Statements\]\(https:\/\/img\.shields\.io\/badge\/Statements-[\d.]+%25-[a-z]+\)/,
      replacement: `![Coverage: Statements](${createBadgeUrl('Statements', `${coverageStats.statements}%`, getCoverageColor(parseFloat(coverageStats.statements)))})`,
    },
    {
      pattern: /!\[Coverage: Branches\]\(https:\/\/img\.shields\.io\/badge\/Branches-[\d.]+%25-[a-z]+\)/,
      replacement: `![Coverage: Branches](${createBadgeUrl('Branches', `${coverageStats.branches}%`, getCoverageColor(parseFloat(coverageStats.branches)))})`,
    },
    {
      pattern: /!\[Coverage: Functions\]\(https:\/\/img\.shields\.io\/badge\/Functions-[\d.]+%25-[a-z]+\)/,
      replacement: `![Coverage: Functions](${createBadgeUrl('Functions', `${coverageStats.functions}%`, getCoverageColor(parseFloat(coverageStats.functions)))})`,
    },
    {
      pattern: /!\[Coverage: Lines\]\(https:\/\/img\.shields\.io\/badge\/Lines-[\d.]+%25-[a-z]+\)/,
      replacement: `![Coverage: Lines](${createBadgeUrl('Lines', `${coverageStats.lines}%`, getCoverageColor(parseFloat(coverageStats.lines)))})`,
    },
  ]
