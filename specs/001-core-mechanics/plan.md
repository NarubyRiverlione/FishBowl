...
**Performance Goals**: 60 FPS rendering, 1 tick/second simulation. Tank tiers target capped capacities (BOWL = 1, STANDARD = 10, BIG = 20); the system should support multiple tanks (2–3) for a typical total capacity in the 40–60 fish range depending on owned tanks. Add measurable profiling targets in Phase 2 (FPS, main-thread ms, memory per fish).
**Constraints**: Real-time simulation (no offline progression), client-side only (no backend for MVP)  
**Scale/Scope**: Single player, multi-tank support (2–3 tanks), STANDARD = 10 fish (BOWL = 1, BIG = 20). Typical total capacity 40–60 fish depending on owned tanks, 5-10 minute play sessions
